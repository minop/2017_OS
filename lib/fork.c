...
pgfault(struct UTrapframe *utf)
{
	...
	if((FEC_WR & err) != FEC_WR) panic("fork pgfault vynimka nebola vyvolana pokusom o zapis, PR: %d WR %d U %d, addr 0x%08x", err&FEC_PR, err&FEC_WR, err&FEC_U, addr);

	// (2) ide o COW stranku?
	if( (uvpd[PDX(addr)] & PTE_P) == 0) /* pritomna? */ panic("fork pgfault stranka nie je mapovana");
	if( (uvpt[PDX(addr)*1024 + PTX(addr)] & (PTE_COW | PTE_P)) != (PTE_COW | PTE_P)) /* krava? */ panic("fork pgfault stranka nie je copy-on-write");
	...
	int perm = ((uvpt[PDX(addr)*1024 + PTX(addr)] & PTE_SYSCALL) & ~PTE_COW) | PTE_W; // povolene flagy bez _COW s _W

	r = sys_page_alloc(thisenv->env_id, (void*)PFTEMP, perm);
	if(r < 0) panic("fork pgfault chyba pri alokovani docastnej stranky; %e", r);
	
	// krok 2 skopirovat obsah
	memcpy((void*)PFTEMP, (void*)PTE_ADDR(addr), PGSIZE); // addr nemusi byt na zaciatku stranky (ten cas, kym som na to prisiel), makro zisti adresu stranky (z inc/mmu.h)

	// krok 3 namapovat docasnu stranku na 'addr'
	r = sys_page_map(thisenv->env_id, (void*)PFTEMP, thisenv->env_id, (void*)PTE_ADDR(addr), perm); // mapovat musim na page-alligned adresy
	if(r < 0) panic("fork pgfault chyba pri vytvarani mapovania; %e", r);
	...
	if(r < 0) panic("fork pgfault chyba pri uvolnovani docasnej stranky; %e", r);
}
...
duppage(envid_t envid, unsigned pn)
{
	...
	// LAB 4: Your code here.
	// chcem si zistit prava => dostat sa k stranke
	if( (uvpd[PDX(pn*PGSIZE)] & PTE_P) == 0) return 0; // stranka, ktora nie je namapovana je uspesne skopirovana
	if( (uvpt[PDX(pn*PGSIZE)*1024 + PTX(pn*PGSIZE)] & (PTE_P | PTE_U)) != (PTE_P | PTE_U)) return 0; // pokial stranka neexistuje (obvious) ale aj ked existuje ale uzivatel k nej nema pristup (systemove volanie je naprogramovane tak, ze musi mat) stranku nebudeme kopirovat

	int perm = (uvpt[PDX(pn*PGSIZE)*1024 + PTX(pn*PGSIZE)]) & PTE_SYSCALL; // len bity povolene pre systemove volania
	...
	if(r < 0) panic("fork duppage chyba pri vytvarani mapovania; %e", r);

	// zmenim prava u seba, tak aby som mal priznak _COW pokial som ho pridaval v kopii
	if(cow) {
		r = sys_page_map(thisenv->env_id, (void*)(pn*PGSIZE), thisenv->env_id, (void*)(pn*PGSIZE), perm); // prva vec, tkora mi napadla. Je mozne, ze sa to da aj jednoduchsie
		if(r < 0) panic("fork duppage chyba pri zmene prav mapovanim; %e", r);
	}
	return 0;
}
...
cprintf("0x%08x\n", &thisenv);
	...
	r = sys_env_set_pgfault_upcall(dieta, pgfault);
	if(r < 0) panic("fork chyba pri nastavovani funkcie na obsluhu vypadku stranky pre detske prostredie; %e", r);

	// namapovanie zasobnika pre obsluhu
	r = sys_page_alloc(dieta, (void*)(UXSTACKTOP-PGSIZE), PTE_P | PTE_U | PTE_W);	
	if(r < 0) panic("fork chyba pri mapovani stranky zasobnika pre detske prostredie; %e", r);
	...
	if(r < 0) panic("fork chyba pri zmene statusu dietata na runnable; %e", r);
	return dieta;
}
